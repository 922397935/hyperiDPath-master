...
class ProteinToProtein(HyperedgeToNode):
    ...
    def _nodes(self):
        ...
        for node in list(zip(self.df["node_1"], self.df["node_2"],
                             self.df["node_1_type"], self.df["node_2_type"],
                             self.df["node_1_name"], self.df["node_2_name"])):
            nodes.append((node[0], {"node_type": node[2], "node_name": node[4]}))
            nodes.append((node[1], {"node_type": node[3], "node_name": node[5]}))
        return nodes
    ...
    def _hyperedges(self):
        ...
        hyperedge_id = ["ppi: " + str(item) for item in _hyperedge_id]
        hyperedges = list()
        for hyperedge in list(zip(bipartite_edgelist, hyperedge_id, hyperedge_id)):
            hyperedges.append((hyperedge[0], f"{hyperedge[1]}", {"hyperedge_type": "ppi"}))
        return hyperedges
