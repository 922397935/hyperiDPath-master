...
def get_path_node_weight(node_map_dict, data, index):
    ...
    disease = node_map_dict[data['indication'][index]]
...
def process_one_dataset(node_map_dict, data, K=5):    
    ...
    for i in tqdm(range(len(data['indication']))):
    ...
    df_dict = {'drug': data['drug'].tolist(),
               'indication': data['indication'].tolist(),
    ...
    output_df['indication'] = output_df['indication'].map(node_map_dict)
    return output_df
