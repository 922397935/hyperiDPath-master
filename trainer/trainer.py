...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, (_, _, path_feature, type_feature, lengths, mask, target) in enumerate(self.data_loader):
            ...
            # TODO: 这里恐怕是需要大改下，根据模型的不同，gcn的参数要实时变动
            # only run gcn in first batch
            hypergcn = True if batch_idx == 0 else False
    
            self.optimizer.zero_grad()
            # 模型的执行
            
            output, _, _, _ = self.model(path_feature, type_feature, lengths, mask, hypergcn)
    ...
    def _valid_epoch(self, epoch):
        ...
        with torch.no_grad():
            for batch_idx, (_, _, path_feature, type_feature, lengths, mask, target) in enumerate(self.valid_data_loader):
                ...
                output, _, _, _ = self.model(path_feature, type_feature, lengths, mask, hypergcn=False)
                loss = self.criterion(output, target)
